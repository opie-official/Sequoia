 ...
 * @param holder
 */
async function customPrompt(text, holder = "") {
 ...
 * @param spaces
 */
async function paintSpaces(spaces) {
    const { __API__ } = window;
    const settings = await __API__.getSettings();
    ...
    const data = document.getElementById("space-name");
 ...
 * @param path
 */
function spaceFabric(parent, index, name, path) {
    const body = document.createElement('div');
...
async function select(body) {
    ...
    const p = body.querySelector(".space-element-number");
